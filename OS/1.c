...
{
    int i, j, in_fr, ex_fr, flag[30] = {0}; // in_fr : internal fragmebtation, ex_fr : external fragmentation
    in_fr = ex_fr = 0;
    for (i = 0; i < np; i++) // loop to check space for process
    {
        for (j = 0; j < nm; j++)
        {
            ...
            * process size is smaller than the memory hole size, just place it and move to next process
            block.
            */
            if (flag[j] == 0 && msize[j] >= psize[i]) // initially flag[] = NULL or 0, after every condition fullfilment flag[j] = 1
                {
                    flag[j] = 1;
                    in_fr = in_fr + msize[j] - psize[i]; // calculate internal fragmentation (left over space after placing process)
                    break;
                }
        }
    ...
    for (i = 0; i < nm; i++) // calculate external fragmenation, its is noting but the left over memory hole in the memory block.
        {
            if (flag[i] == 0)
                ex_fr = ex_fr + msize[i];
        }
