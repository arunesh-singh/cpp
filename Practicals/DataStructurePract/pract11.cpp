...
public:
    ...
    void mergeHelper(Node<int> *&node)
    {
        ...
        // no right child - single child     if (node->right == nullptr)       node = node->left;

        // no left child - single chold     else if (node->left == nullptr)       node = node->right;

        // node has both children     else
        {
            // find in-order predecessor       temp = node->left;       while (temp->right != nullptr)         temp = temp->right;       // merge subtree to predecessor       temp->right = node->right;       temp = node;       node = node->left;
        }
    ...
    void copyHelper(Node<int> *&node)
    {
        ...
        // find the in-order predecessor       temp = node->left;       while (temp->right != nullptr)
        {
            prev = temp;
            temp = temp->right;
        }
        // copy the prdecessor key       node->data = temp->data;       // handle dangling subtrees       if (prev == node)         prev->left = temp->left;       else
...
int main(void)
{
    ...
    do
    {
        cout << "      MENU      \n"
             ...
             << "(1) Insertion\n"
             << "(2) Searching a node\n"
             << "(3) Display its preorder, postorder and inorder traversals. (recu rsive)\n"
             << "(4) Display its preorder, postorder and inorder traversals. (iter ative)\n"
             ...
             << "    and then place the node with value y at its appropriate posit ion\n"
        ...
        case 7:
            ...
            cout << "Total Nodes: " << tree.countNonLeaf + tree.countLeaf << endl;
            break;
        case 8:
            cout << "\nEnter Search Data: ";
            cin >> data;
